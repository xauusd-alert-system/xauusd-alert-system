...
class TestScanner(unittest.TestCase):
    ...
    def test_resample(self):
        base = [
            {"time": 100, "open": 1, "high": 2, "low": 0, "close": 1.5, "volume": 1},
    ...
    def _synth_day(pin):
        ...
        for i in range(960):
            ...
            out.append(_bar(ts, round(o, 4), round(h, 4), round(l, 4), round(c, 4), round(_random.uniform(800, 1200))))
            price = c
    ...
    def test_target_rr_config(self):
        ...
        day = self._synth_day((102.85, 103.3, 102.4, 103.0))  # bullish pin, sane levels
        res = scanner_mod.scan_setup("SYN", dt.date(2026, 8, 20), day, dt.time(13, 30), {"target_rr": 3.5})
        self.assertTrue(res.tradable)
    ...
    def test_signal_dead_zone_rejected(self):
        ...
        self.assertTrue(60.0 <= sig_min <= 69.0, f"сигнал на {sig_min:.1f} мин — тест ждёт мёртвую зону 60-69")
        res = scanner_mod.scan_setup("SYN", dt.date(2026, 8, 20), day, dt.time(13, 30), {"signal_dead_zone": [60, 69]})
        self.assertFalse(res.tradable)
    ...
    def test_dead_day_rejected(self):
        ...
        res = scanner_mod.scan_setup("SYN", dt.date(2026, 8, 20), d1 + d2, dt.time(13, 30), {})
        self.assertFalse(res.tradable)
...
class TestOutcomes(unittest.TestCase):
    ...
    def test_stop(self):
        sig = self._ts(14, 30)
        bars = [self._bar(sig + 60, 100.0, 100.4, 99.9, 100.0), self._bar(sig + 120, 100.0, 100.2, 99.4, 99.5)]
    ...
    def test_target_full_position(self):
        ...
        sig = self._ts(14, 30)
        bars = [
            self._bar(sig + 60, 100.0, 100.5, 100.2, 100.4),  # +1R — не выход
            self._bar(sig + 120, 100.4, 101.1, 100.8, 101.0),
        ]  # +2R — тейк
        out, r, _ = outcomes_mod.simulate_outcome(sig, 100.0, 99.5, 101.0, "long", bars)
    ...
    def test_target_short(self):
        ...
        bars = [self._bar(sig + 60, 100.0, 100.3, 99.8, 99.9), self._bar(sig + 120, 99.9, 100.1, 98.9, 99.0)]
    ...
    def test_pending_then_eod(self):
        ...
        out, _, _ = outcomes_mod.simulate_outcome(sig, 100.0, 99.5, 101.0, "long", bars, now_ts=self._ts(15, 0))
        self.assertIsNone(out)
        # после 19:55 -> EOD по цене закрытия (= входу -> R=0)
        out, r, _ = outcomes_mod.simulate_outcome(sig, 100.0, 99.5, 101.0, "long", bars, now_ts=self._ts(20, 0))
...
class TestJournal(unittest.TestCase):
    def test_roundtrip(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            n1 = journal_mod.add_trade(p, "2026-08-19", "14:30", "AAPL", "L", "B", 318.65, 312.30, 331.33, 2.5, 0.25)
            n2 = journal_mod.add_trade(p, "2026-08-19", "14:40", "NVDA", "S", "C", 219, 220, 217, 2.5, 0.25)
