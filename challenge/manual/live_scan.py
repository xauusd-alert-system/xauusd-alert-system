...
def get_candles(access, symbol_id, interval="Min1", candles_count=720, to=None):
    ...
    r = requests.post(
        GRPC_BASE + "MobileDataService.getCandlesToDate",
...
def main():
    ...
    print(f"UTC now: {dt.datetime.now(dt.UTC):%Y-%m-%d %H:%M:%S}  session date: {today}", file=sys.stderr)
    ...
    for sym in watch:
        ...
        if res.tradable:
            hits += 1
            print(
                f"TRADE {res.bias.upper()} {sym} @ {res.entry:.2f} stop {res.stop:.2f} "
    ...
    if hits == 0:
        print("No A/B setups right now. Re-run during/after the impulse window (first 60-90 min of the session).")
