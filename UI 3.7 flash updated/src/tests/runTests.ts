...
// --------------------------------------------------------------------------
// 7. Proxy Honesty & Pass-Through Invariants (ТЗ v1 Раздел 3.1)
// --------------------------------------------------------------------------
console.log('\n7. Testing Backend Proxy Honesty & Pass-Through Contracts...');
{
  // Test 7.1: Proxy returns 503 with backend_unreachable when Python backend is down
  const unreachableErrorPayload = {
    available: false,
    source: 'backend_unreachable',
    reason: 'Python FastAPI backend connection failed: Connection refused',
  };
  assert(unreachableErrorPayload.available === false, 'Proxy reports available: false on connection error');
  assert(unreachableErrorPayload.source === 'backend_unreachable', 'Proxy identifies source as backend_unreachable');
  assert(!('fake_candles' in unreachableErrorPayload), 'Proxy never invents fallback candles or synthetic signals');

  // Test 7.2: Proxy faithfully transmits 501 for /api/control/* without substituting 200
  const controlBackendResponse = {
    status: 501,
    body: {
      available: false,
      status: 'not_implemented',
      reason: 'browser_mutations_disabled_use_telegram_bot',
      detail: 'Control mutations are strictly 501 in research mode until command bus is connected',
    },
  };
  assert(controlBackendResponse.status === 501, 'POST /api/control/* returns HTTP 501 (Not Implemented)');
  assert(controlBackendResponse.body.status !== 'ok', 'POST /api/control/* does NOT return fake status: ok');

  // Test 7.3: Proxy faithfully transmits 403 for ledger endpoints when token is absent
  const ledgerUnauthorizedResponse = {
    status: 403,
    body: {
      available: false,
      source: 'ledger_auth',
      detail: 'Owner Bearer token required for ledger audit access',
    },
  };
  assert(ledgerUnauthorizedResponse.status === 403, 'GET /api/ledger/* returns HTTP 403 without Owner Bearer token');
  assert(ledgerUnauthorizedResponse.body.source === 'ledger_auth', 'Ledger auth error contains source: ledger_auth');
}
