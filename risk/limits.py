...
class RiskLimits:
    ...
    def __init__(
        ...
        self.max_concurrent_positions = int(exec_cfg.get("max_concurrent_positions_global", 3))
        ...
        self.max_open_positions_per_asset = int(exec_cfg.get("max_open_positions_per_asset", 2))
        self.max_daily_trades_per_asset = int(exec_cfg.get("max_daily_trades_per_asset", 10))
        self.max_daily_loss_pct = float(cfg.get("backtest", {}).get("max_daily_loss_pct", 5.0))
        ...
        self.exclude_swaps = bool(cfg.get("risk", {}).get("circuit_breaker", {}).get("exclude_swaps", True))

    # --------------------------------------------------- daily-budget gate
    def _reset_daily_stats_if_needed(self, current_equity: float, current_balance: Optional[float] = None) -> None:
        ...
        if not st.is_today() or st.starting_equity_today is None:
            ...
            logger.info("🛡 Risk Manager Reset for %s. Starting Daily Equity: $%.2f", st.current_day, current_equity)

    def daily_pnl(self, current_equity: float, current_balance: float) -> float:
        ...
        if self.exclude_swaps:
            ...
            trading_balance_delta = current_balance - (st.starting_balance_today or 0.0)
            return trading_balance_delta + floating_pnl
        return current_equity - (st.starting_equity_today or 0.0)

    def check_circuit_breaker(self, current_equity: float, current_balance: float) -> tuple[bool, str]:
        """Daily-loss circuit breaker. Persists the trip flag (W10)."""
        self._reset_daily_stats_if_needed(current_equity, current_balance)
        st = self.state

        max_allowed_loss = (st.starting_equity_today or 0.0) * (self.max_daily_loss_pct / 100.0)
    ...
    def check_concurrency(
        self, asset_key: str, groups_by_asset: Optional[dict] = None, singles_by_asset: Optional[dict] = None
    ) -> tuple[bool, str]:
        ...
        if groups_by_asset is not None and singles_by_asset is not None:
            total_groups = sum(len(keys) for keys in groups_by_asset.values()) + sum(singles_by_asset.values())
            if total_groups >= self.max_concurrent_positions:
                return False, (f"Max concurrent groups limit reached ({total_groups}/{self.max_concurrent_positions})")
            asset_groups = len(groups_by_asset.get(asset_key, ())) + singles_by_asset.get(asset_key, 0)
            if asset_groups >= self.max_open_positions_per_asset:
                return False, (
                    f"Max open groups for {asset_key} reached ({asset_groups}/{self.max_open_positions_per_asset})"
                )
        else:
            open_positions = self._positions()
            if open_positions and len(open_positions) >= self.max_concurrent_positions:
                return False, (
                    f"Max concurrent positions limit reached ({len(open_positions)}/{self.max_concurrent_positions})"
                )
        return True, "OK"
    ...
    def check_daily_trades(self, asset_key: str) -> tuple[bool, str]:
        ...
        if asset_trades >= self.max_daily_trades_per_asset:
            return False, (
                f"Daily trade limit for {asset_key} reached ({asset_trades}/{self.max_daily_trades_per_asset})"
            )
        return True, "OK"
    ...
    def can_trade(
        ...
        ok, reason = self.check_circuit_breaker(current_equity, current_balance)
        if not ok:
            return False, reason

        ok, reason = self.check_concurrency(asset_key, groups_by_asset, singles_by_asset)
        if not ok:
            return False, reason
