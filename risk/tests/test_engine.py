...
def _engine(cfg=None, state_path="unused.json", **kw):
    cfg = cfg or _cfg()
    return RiskEngine(cfg, magic=777111, state_path=state_path,
                      legacy_throttle=False, **kw)
...
def test_can_open_aggregates_all_gates(tmp_path):
    ...
    ok, reason = eng.can_open("XAUUSD", equity=10_000.0, balance=10_000.0,
                              use_legacy_throttle=False)
    assert ok, reason

    # Gate 1: circuit breaker (daily loss beyond 5% of 10_000 = 500).
    ok, reason = eng.can_open("XAUUSD", equity=9_400.0, balance=9_400.0,
                              use_legacy_throttle=False)
    assert not ok
    ...
    ok, reason = eng2.can_open(
        "XAUUSD", equity=10_000.0,
        groups_by_asset={"XAGUSD": {"G1"}, "BTCUSD": {"G2"}},
        singles_by_asset={}, use_legacy_throttle=False)
    assert not ok
    ...
    ok, reason = eng3.can_open("XAUUSD", equity=10_000.0,
                               use_legacy_throttle=False)
    assert not ok
    assert "Daily trade limit" in reason

    # Gate 4: rate throttle (2 orders/min cap, window full).
    eng4 = _engine(state_path=str(tmp_path / "s4.json"),
                   rate_throttle=RateThrottle(max_orders_per_minute=2))
    eng4.rate_throttle.record_order("XAUUSD")
    eng4.rate_throttle.record_order("XAUUSD")
    ok, reason = eng4.can_open("XAUUSD", equity=10_000.0,
                               use_legacy_throttle=False)
    assert not ok
    ...
    ok, reason = eng5.can_open("XAUUSD", equity=9_100.0, balance=9_100.0,
                               use_legacy_throttle=False)
    assert not ok
    ...
    ok, reason = eng6.can_open("XAUUSD", equity=10_000.0,
                               cluster="metals", add_risk_pct=0.005,
    ...
    with pytest.raises(TypeError):
        eng6.can_open("XAUUSD", equity=10_000.0, cluster="metals",
                      add_risk_pct=0.001, use_legacy_throttle=False)
...
def test_circuit_breaker_blocks_until_next_day(state_path):
    ...
    eng = _engine(state_path=state_path)
    assert eng.can_open("XAUUSD", equity=10_000.0, balance=10_000.0,
                        use_legacy_throttle=False)
    ...
    ok, reason = eng.can_open("XAUUSD", equity=11_000.0, balance=11_000.0,
                              use_legacy_throttle=False)
    assert not ok
    assert "Circuit Breaker" in reason
    ...
    eng.state.current_day = (
        engine_mod.datetime.now(engine_mod.timezone.utc).date()
        - timedelta(days=1))
    ok, reason = eng.can_open("XAUUSD", equity=11_000.0, balance=11_000.0,
                              use_legacy_throttle=False)
    assert ok, reason
...
def test_drawdown_throttle_recovers(state_path):
    ...
    # -9% from HWM -> blocked.
    ok, reason = eng.can_open("XAUUSD", equity=9_100.0, balance=9_100.0,
                              use_legacy_throttle=False)
    assert not ok
    assert "drawdown_throttle" in reason
    ...
    ok, reason = eng.can_open("XAUUSD", equity=9_500.0, balance=9_500.0,
                              use_legacy_throttle=False)
    assert ok, reason

    # New equity high -> HWM ratchets up; a subsequent drop measured against
    # the NEW HWM.
    ok, _ = eng.can_open("XAUUSD", equity=10_500.0, balance=10_500.0,
                         use_legacy_throttle=False)
    assert eng.state.hwm == 10_500.0
    ok, reason = eng.can_open("XAUUSD", equity=10_500.0 * 0.91,
                              balance=10_500.0 * 0.91,
                              use_legacy_throttle=False)
    assert not ok
    assert "drawdown_throttle" in reason
...
def test_hwm_persisted_and_updated(state_path, tmp_path):
    ...
    ok, _ = eng.can_open("XAUUSD", equity=10_000.0, balance=10_000.0,
                         use_legacy_throttle=False)
    assert eng.state.hwm == 10_000.0

    # Drawdown to 9_800 -> HWM NOT lowered.
    eng.can_open("XAUUSD", equity=9_800.0, balance=9_800.0,
                 use_legacy_throttle=False)
    assert eng.state.hwm == 10_000.0

    # New high -> ratchet.
    eng.can_open("XAUUSD", equity=10_200.0, balance=10_200.0,
                 use_legacy_throttle=False)
    ...
    ok, reason = eng2.can_open("XAUUSD", equity=10_200.0 * 0.91,
                               balance=10_200.0 * 0.91,
                               use_legacy_throttle=False)
    assert not ok
    assert "drawdown_throttle" in reason
...
def test_no_daily_limits_in_throttle():
    ...
    for forbidden in ("max_trades_per_day", "daily_trades_count",
                      "trades_today", "max_daily_loss_pct",
                      "on_trade_closed", "check_daily_trades"):
...
def test_engine_aliases_and_summary(state_path):
    ...
    eng = _engine(state_path=state_path)
    ok_alias, _ = eng.can_trade("XAUUSD", equity=10_000.0, balance=10_000.0,
                                use_legacy_throttle=False)
    ok_open, _ = eng.can_open("XAUUSD", equity=10_000.0, balance=10_000.0,
                              use_legacy_throttle=False)
...
def test_engine_state_file_backwards_compat(state_path):
    ...
    ok, _ = eng.can_open("XAUUSD", equity=5_000.0, balance=5_000.0,
                         use_legacy_throttle=False)
    assert ok
