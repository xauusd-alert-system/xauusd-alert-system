...
class ChartRenderer:
    ...
    def render_svg_candlestick(
        ...
        for idx, row in slice_df.iterrows():
            ...
            svg.append(f'<rect x="{cx - candle_w*0.35}" y="{top_b}" width="{candle_w*0.7}" height="{body_h}" fill="{color}"/>')

        # Draw Trade Levels
        if entry_price:
            ey = scale_y(entry_price)
            svg.append(f'<line x1="20" y1="{ey}" x2="{width-20}" y2="{ey}" stroke="#38bdf8" stroke-width="2" stroke-dasharray="4,4"/>')
            svg.append(f'<text x="25" y="{ey-5}" fill="#38bdf8" font-size="11" font-weight="bold">ENTRY: {entry_price:.2f}</text>')

        if sl_price:
            sy = scale_y(sl_price)
            svg.append(f'<line x1="20" y1="{sy}" x2="{width-20}" y2="{sy}" stroke="#f43f5e" stroke-width="2"/>')
            svg.append(f'<text x="25" y="{sy-5}" fill="#f43f5e" font-size="11" font-weight="bold">STOP: {sl_price:.2f}</text>')

        if tp_prices:
            for i, tp in enumerate(tp_prices):
                ty = scale_y(tp)
                svg.append(f'<line x1="20" y1="{ty}" x2="{width-20}" y2="{ty}" stroke="#10b981" stroke-width="1.5" stroke-dasharray="2,2"/>')
                svg.append(f'<text x="{width-120}" y="{ty-5}" fill="#10b981" font-size="11" font-weight="bold">TP{i+1}: {tp:.2f}</text>')
