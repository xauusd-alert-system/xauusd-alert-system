...
def test_signal_spec_supports_arbitrary_target_legs_and_latency():
    spec = SignalSpec(
        ...
        created_at_utc=100,
        published_at_utc=112,
        zone_low=2000,
        zone_high=2001,
        stop_price=1990,
        targets=[
            TargetLeg(price=2010, close_ratio=0.4),
            TargetLeg(price=2020, close_ratio=0.3),
            TargetLeg(price=2030, close_ratio=0.2),
            TargetLeg(price=2040, close_ratio=0.1),
        ],
        confirmed_by="systematic:ensemble",
        config_hash="abc",
    )
...
def test_primary_event_ledger_is_chained_and_append_only(tmp_path):
    ...
    common = dict(
        db_path=db, signal_id="s1", asset_key="XAUUSD", strategy_version="v3", config_hash="cfg", actor="test"
    )
...
def test_telegram_archive_stays_unlinked_and_idempotent(tmp_path):
    ...
    html.write_text(
      ...
      <div class="text">#hehe XAUUSD TP1</div></div>""",
        encoding="utf-8",
    )
...
def test_historical_news_calendar_is_dated_and_symmetric(tmp_path):
    ...
    blocked, reason, available = news_guard_decision(100000, historical_calendar_path=str(path))
