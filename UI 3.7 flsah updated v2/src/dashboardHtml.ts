    ...
    <script>
        ...
        async function refreshAllData() {
            ...
            if (st && !st._error && st.available !== false) {
                ...
                document.getElementById('sys-health-status').innerHTML = '<i class="fas fa-check-circle text-emerald-400"></i> <span class="text-emerald-400 font-bold">' + (st.freshness_status || 'FRESH').toUpperCase() + '</span>';
            } else {
            ...
            if (sig && !sig._error && sig.available !== false) {
                ...
                document.getElementById('sig-freshness-badge').innerHTML = '<i class="fas fa-check-circle text-emerald-400"></i> <span class="text-emerald-400 font-bold">' + (sig.freshness_status || 'FRESH').toUpperCase() + '</span>';
                document.getElementById('sig-envelope-lineage').innerText = 'Source: ' + (sig.source || 'realtime_pipeline') + ' | Mode: ' + (sig.mode || 'live_verified') + ' | As of: ' + (sig.as_of_utc || '—');
                ...
                document.getElementById('sig-envelope-lineage').innerText = 'Source: unavailable | Mode: offline | As of: —';
                var decBadge = document.getElementById('sig-decision-badge');
