...
* **WebSocket стратегия:** **Variant B (Honest REST-Only Mode)**.
  - Node UI (порт 3000) не имитирует WebSocket-поток и не искажает события.
  - Web UI использует периодический REST polling (`GET /api/ledger/events?limit=50`) с передачей `Authorization: Bearer <token>`.
  - При прямом обращении к `/ws` на порт 3000 возвращается честный `HTTP 501` (`ws_not_implemented`).
  - Для прямого WS-подключения клиент может подключаться напрямую к Python FastAPI WS-серверу (`ws://127.0.0.1:8000/ws?token=...`).
