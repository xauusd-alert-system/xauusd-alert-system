...
| `/api/sentiment` sample headlines | Реальный источник либо `unavailable` | ✅ **Обновлено (2026-08-18)**: считается из реального Forex Factory High-Impact календаря (`data/news_filter.py`) лексиконом `data/sentiment_analyzer.py`; `available=false, reason=news_feed_unavailable` + `freshness_status=waiting` при недоступном фиде. Никаких sample-заголовков. |
