...
- VWAP считается от открытия регулярной сессии 09:30 America/New_York,
  типичная цена `(H+L+C)/3`, сброс в новый торговый день.
- OR15 = первые **три полностью закрытые** 5m-свечи (09:30–09:45).
...
`PERSONAL_DAILY_STOP (-$20)` · `MAX_TRADES_REACHED (2)` ·
`MAX_CONSECUTIVE_LOSSES (2)` · `DAILY_PROFIT_LOCK (+$20)` ·
`SESSION_CLOSE_GUARD (25 мин до закрытия)` · `ACTIVE_POSITION_EXISTS` ·
`DAY_STOPPED (оператор)`. Порядок проверки детерминирован; отказы журнализируются.
