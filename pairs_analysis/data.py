    ...
    try:
        ...
        try:
            row = con.execute(
                ...
                "FROM mt5_cache WHERE cache_key=? ORDER BY open_time",
                (key,)).fetchall()
            if not row:
                return None
...
def _mt5_disk_cache_put(df: pd.DataFrame, key: str,
                         cache_path: str = DEFAULT_CACHE) -> None:
    ...
    try:
        ...
        con.executemany(
            "INSERT INTO mt5_cache (cache_key, open_time, open, high, low, close, volume) "
            "VALUES (?,?,?,?,?,?,?)",
...
def load_mt5(symbol: str, timeframe: str = "D1",
             db_path: str = DEFAULT_DB,
             _cache_path: str = DEFAULT_CACHE) -> pd.DataFrame:
    ...
    try:
        ...
        rows = cur.execute(
            f"SELECT timestamp_utc, open, high, low, close, volume FROM {table} "
            f"WHERE symbol=? ORDER BY timestamp_utc", (symbol,)).fetchall()
    finally:
        con.close()
...
def _fetch_binance_pages(symbol: str, interval: str,
...
def fetch_binance(symbol: str, timeframe: str = "D1",
                  start: dt.date | dt.datetime | None = None,
                  end: dt.date | dt.datetime | None = None,
                  cache_path: str = DEFAULT_CACHE,
                  lookback_days: int = 800) -> pd.DataFrame:
    ...
    try:
        rows = con.execute(
            ...
            "ORDER BY open_time",
            (symbol, interval, start_ms, end_ms)).fetchall()
        ...
        covered = (len(rows) >= 2 and rows[0][0] <= start_ms + interval_ms
                   and rows[-1][0] >= end_ms - 2 * interval_ms)
        if covered:
    ...
    try:
        con.executemany(
            ...
            "VALUES (?,?,?,?,?,?,?,?)",
            [(symbol, interval, int(ts.timestamp() * 1000),
              float(r.open), float(r.high), float(r.low), float(r.close), float(r.volume))
             for ts, r in df.iterrows()])
        con.commit()
    finally:
