...
def pair_score(m: PairMetrics, sig: Signal, ens: EnsembleForecast, weights: dict | None = None) -> float:
...
def scan_pairs(cfg: dict, thresholds: dict | None = None, timeframe: str = "D1") -> list[PairWatchlistEntry]:
    ...
    for pair in cfg.get("pairs", []):
        ...
        try:
            ...
            entries.append(PairWatchlistEntry(name=name, status="INVALID", direction="none", reason=str(e)))
...
def size_pair_position(
    ...
    return PairPosition(
        ...
        p1_symbol=p1_sym,
        p1_contracts=round(p1_contracts, 4),
        p1_price=m.p1_last,
        p2_symbol=p2_sym,
        p2_contracts=round(p2_contracts, 4),
        p2_price=m.p2_last,
...
PAIR_JOURNAL_DEFAULT = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "manual", "pair_journal.csv"
)
        ...
        out.append(
            {
                ...
                "by_entry_z": {k: {"n": len(v), "avg_r": _avg(v)} for k, v in by_z_buckets.items()},
                "by_regime": {k: {"n": len(v), "avg_r": _avg(v)} for k, v in by_regime.items()},
