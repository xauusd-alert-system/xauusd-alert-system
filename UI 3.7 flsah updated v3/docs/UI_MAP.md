...
* **API endpoint:** `GET /api/ledger/events`, `POST /api/ledger/ingest`
* **WebSocket стратегия:** **Variant B (Honest REST-Only Mode)**.
  - Node UI (порт 3000) не поддерживает WebSocket Upgrade proxy, не имитирует WebSocket-поток и не искажает события.
  - Web UI (Audit timeline) работает через периодический REST polling (`GET /api/ledger/events?limit=50`) с передачей `Authorization: Bearer <token>`.
  - При выполнении HTTP GET запроса к `/ws` на порт 3000 возвращается честный `HTTP 501` (`ws_not_implemented`). Обратите внимание: WebSocket Upgrade handshake на порт 3000 не проксируется; для реального WebSocket-подключения (`/ws?token=...`) клиент при необходимости должен подключаться непосредственно к Python FastAPI backend на `BACKEND_BASE_URL` (например, `ws://127.0.0.1:8000/ws?token=...`), а не к порту Node UI.
* **Signed Ingress (`POST /api/ledger/ingest`):**
  - Прокси передает входящий поток байт (`req.body`) напрямую без десериализации (`express.json()`) и без повторной сериализации.
  - Заголовки `X-Ledger-Signature`, `Authorization`, `Content-Type` транслируются неизменными.
  - Это гарантирует 100% совпадение HMAC-SHA256 подписи в Python `data/ledger_bridge.py`.
