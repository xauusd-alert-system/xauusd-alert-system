...
<body class="p-3 sm:p-5 md:p-8 min-h-screen text-slate-200">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="glass-panel p-6 lg:col-span-2 space-y-5 border-l-4 border-l-amber-500">
                    <div class="flex flex-wrap justify-between items-start gap-3">
                        ...
                        <div class="text-right">
                            ...
                            <div class="text-[11px] text-slate-400 mt-1">
                                Signal State: <span id="sig-state" class="font-mono text-slate-400">WAITING</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-slate-900/60 p-3.5 rounded-lg border border-slate-800 text-xs">
                        <div>
                            ...
                            <span id="sig-bias-text" class="font-mono font-bold text-base text-slate-400">—</span>
                        </div>
                        <div>
                            <span class="text-slate-400 text-[11px] block">ML CONFIDENCE</span>
                            <span id="sig-confidence-val" class="font-mono font-bold text-base text-slate-400">—</span>
                        </div>
                        <div>
                            <span class="text-slate-400 text-[11px] block">MARKET REGIME</span>
                            <span id="sig-regime-val" class="font-mono font-bold text-base text-slate-400">—</span>
                        </div>
                        <div>
                            <span class="text-slate-400 text-[11px] block">DATA FRESHNESS</span>
                            <span id="sig-freshness-badge" class="inline-flex items-center gap-1 font-mono font-bold text-slate-400 text-xs mt-1">
                                <i class="fas fa-hourglass-half"></i> WAITING
                            </span>
                        </div>
                    </div>
                ...
                <div class="glass-panel p-6 space-y-4">
                    ...
                    <div class="text-xs text-slate-300 bg-slate-900/80 p-3 rounded-lg border border-slate-800">
                        ...
                        <p id="sig-reasoning-summary" class="italic text-slate-400 leading-relaxed">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="glass-panel p-6 lg:col-span-2 space-y-4">
                    <div class="flex justify-between items-center">
                        ...
                        <span id="positions-freshness" class="text-xs px-2 py-0.5 rounded font-mono bg-slate-800 text-slate-400">
                            WAITING
                        </span>
                    </div>
            ...
            <div class="glass-panel p-6 space-y-3">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 font-mono text-xs">
                    <div class="p-3 bg-slate-900/60 rounded border border-slate-800">
                        ...
                        <div id="paper-status-val" class="text-sm font-bold text-slate-400 mt-1">LOADING…</div>
                    </div>
                    <div class="p-3 bg-slate-900/60 rounded border border-slate-800">
                        <div class="text-slate-400 text-[11px]">Закрытых сделок</div>
                        <div id="paper-trades-val" class="text-sm font-bold text-slate-400 mt-1">— / —</div>
                    </div>
                    <div class="p-3 bg-slate-900/60 rounded border border-slate-800">
    ...
    <script>
        ...
        async function refreshAllData() {
            ...
            if (st && !st._error && st.available !== false) {
                var dMode = st.data_mode || '—';
                ...
                document.getElementById('sys-health-status').innerHTML = '<i class="fas ' + stIcon + '"></i> <span class="' + stColor + ' font-bold">' + stFresh + '</span>';
            } else {
                document.getElementById('sys-data-mode').innerText = 'UNAVAILABLE';
                document.getElementById('sys-cfg-hash').innerText = '—';
                document.getElementById('badge-deployment-mode').innerText = 'BACKEND OFFLINE (503)';
                document.getElementById('sys-health-status').innerHTML = '<i class="fas fa-power-off text-rose-400"></i> <span class="text-rose-400 font-bold">OFFLINE</span>';
            }

            // Primary Signal
            var sig = await fetchJSON('/signal?asset=XAUUSD', false);
            if (sig && !sig._error && sig.available !== false) {
                document.getElementById('sig-id').innerText = sig.signal_id || '—';
                document.getElementById('sig-session').innerText = sig.session ? sig.session.toUpperCase() : '—';
                document.getElementById('sig-state').innerText = sig.signal_state ? sig.signal_state.toUpperCase() : 'UNAVAILABLE';
                document.getElementById('sig-bias-text').innerText = sig.bias ? sig.bias.toUpperCase() : '—';
                document.getElementById('sig-confidence-val').innerText = sig.confidence != null ? (Number(sig.confidence) * 100).toFixed(1) + '%' : '—';
                document.getElementById('sig-regime-val').innerText = sig.regime ? sig.regime.toUpperCase() : '—';
                ...
                var sigFresh = (sig.freshness_status || 'UNKNOWN').toUpperCase();
                var sigIcon = sigFresh === 'FRESH' ? 'fa-check-circle text-emerald-400' : (sigFresh === 'STALE' ? 'fa-history text-amber-400' : 'fa-info-circle text-slate-400');
                var sigColor = sigFresh === 'FRESH' ? 'text-emerald-400' : (sigFresh === 'STALE' ? 'text-amber-400' : 'text-slate-400');
                document.getElementById('sig-freshness-badge').innerHTML = '<i class="fas ' + sigIcon + '"></i> <span class="' + sigColor + ' font-bold">' + sigFresh + '</span>';
                document.getElementById('sig-envelope-lineage').innerText = 'Source: ' + (sig.source || 'not provided by backend') + ' | Mode: ' + (sig.mode || 'not provided by backend') + ' | As of: ' + (sig.as_of_utc || '—');
                document.getElementById('sig-confirmed-by').innerText = sig.confirmed_by || '—';
                ...
                document.getElementById('sig-id').innerText = '—';
                document.getElementById('sig-session').innerText = '—';
                document.getElementById('sig-state').innerText = 'UNAVAILABLE';
                document.getElementById('sig-bias-text').innerText = '—';
                document.getElementById('sig-confidence-val').innerText = '—';
                document.getElementById('sig-regime-val').innerText = '—';
                document.getElementById('sig-step').innerText = '—';
                document.getElementById('sig-reasoning-summary').innerText = 'Бэкенд недоступен (503 Service Unavailable / backend_unreachable)';
                document.getElementById('sig-freshness-badge').innerHTML = '<i class="fas fa-power-off text-rose-400"></i> <span class="text-rose-400 font-bold">OFFLINE</span>';
                document.getElementById('sig-envelope-lineage').innerText = 'Source: not provided by backend | Mode: offline | As of: —';
                document.getElementById('sig-confirmed-by').innerText = '—';
                var decBadge = document.getElementById('sig-decision-badge');
                decBadge.className = 'inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-bold bg-slate-800 text-slate-400 border border-slate-700';
                decBadge.innerHTML = '<i class="fas fa-ban"></i> UNAVAILABLE';
                document.getElementById('sig-entry').innerText = '—';
                document.getElementById('sig-sl').innerText = '—';
                document.getElementById('sig-tp1').innerText = '—';
                document.getElementById('sig-tp2').innerText = '—';
                document.getElementById('sig-tp3').innerText = '—';
            }
            ...
            var paper = await fetchJSON('/api/paper-status', false);
            if (paper && paper.available) {
                var pSt = (paper.status || 'ACCUMULATING').toUpperCase();
                var closed = paper.closed_trades_count != null ? paper.closed_trades_count : 0;
                var minTrades = paper.min_closed_trades != null ? paper.min_closed_trades : 50;
                document.getElementById('paper-status-val').innerText = pSt;
                document.getElementById('paper-trades-val').innerText = closed + ' / ' + minTrades + ' MIN';
            } else {
