...
def detect_structure(df: pd.DataFrame, lookback: int = 20) -> pd.DataFrame:
    ...
    # Confirm swing at candidate index c only once we have `lookback` candles AFTER c (i.e. current index i = c + lookback)
    ...
    confirmed_high_level = pd.Series(np.where(swing_high, highs[np.maximum(np.arange(n) - lookback, 0)], np.nan), index=out.index)
    confirmed_low_level = pd.Series(np.where(swing_low, lows[np.maximum(np.arange(n) - lookback, 0)], np.nan), index=out.index)
