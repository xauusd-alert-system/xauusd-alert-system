...
WHITE_LIST: dict[str, str] = {
    ...
    "scripts/run_simulation.py": "TODO(8.6): shim entry point; sys.path injection requires raw import",
    # Same shim-injection role as run_simulation (virtual-mode bot entry).
    "scripts/run_bot.py": "TODO(8.6): shim entry point; sys.path injection requires raw import",
    # Ops/admin bot: reads MT5 status ad-hoc; convert to MT5Client when the
    # admin bot gets a DI path (low risk, cosmetic).
    "scripts/telegram_admin.py": "TODO(8.6): convert status reads to MT5Client",
    # Alerts control bot: convert to MT5Client once control_bot gets a DI
    # constructor parameter (protects closeall path).
    "alerts/control_bot.py": "TODO(8.6): convert /positions and /closeall to MT5Client",
}

_IMPORT_RE = re.compile(r"^\s*(?:import\s+(?:MetaTrader5|mt5)\b|from\s+(?:MetaTrader5|mt5)\b)")
...
def test_white_list_entries_exist():
    ...
    missing = [rel for rel in WHITE_LIST if not (PROJECT_ROOT / rel).exists()]
...
def test_white_list_is_minimal_and_documented():
    ...
    for rel, note in WHITE_LIST.items():
        assert note.startswith("TODO"), f"White-list entry {rel!r} must carry a TODO rationale"
