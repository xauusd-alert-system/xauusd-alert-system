...
def calibrate_with(base_template, X_train, y_train, cfg, min_fit, min_calib, horizon):
    ...
    if n < min_fit + horizon + min_calib + 1:
        print(
            f"    !! too small for a purged calibration split "
...
def main():
    ...
    ap.add_argument(
        "--embargo", type=int, default=None, help="override backtest.walk_forward.embargo_candles for variant C"
    )
    ...
    try:
        ...
        df = build_full_df(raw, cfg, db_path=db_path, asset_key=asset_key, timeframe=args.timeframe)
        ...
        embargo = (
            ...
            else int(cfg.get("backtest", {}).get("walk_forward", {}).get("embargo_candles", 0))
        )
        ...
        print(f"train (plain) : {len(Xtr_p)}    train (purged): {len(Xtr_g)}    dropped: {len(Xtr_p) - len(Xtr_g)}")
        ...
        specs = [
            (
                ...
                Xtr_p,
                ytr_p,
                max(30, min(200, len(Xtr_p) // 3 // 2)),
                max(20, min(100, len(Xtr_p) // 6 // 2)),
            ),
            ("B +holdout", Xtr_p, ytr_p, max(30, int(len(Xtr_p) * 0.60)), max(20, int(len(Xtr_p) * 0.15))),
            ("C +purge", Xtr_g, ytr_g, max(30, int(len(Xtr_g) * 0.60)), max(20, int(len(Xtr_g) * 0.15))),
        ]
        ...
        print(f"{'variant':<12} {'auc':>8} {'accuracy':>9} {'brier':>8} {'ece':>8} {'p_mean':>8} {'p_std':>8}")
        print("-" * 68)
        for r in rows:
            print(
                f"{r['name']:<12} {r['auc']:>8.4f} {r['accuracy']:>9.4f} "
        ...
        thresholds = [
            ...
            (
                ...
                float(acfg.get("min_confidence_to_alert", ens.get("min_confidence_to_alert", 0.60))),
            ),
        ]
        yt_arr = np.asarray(yte, dtype=int)
        for label, thr in thresholds:
            print(f"\n  {label} = {thr}")
            print(f"    {'variant':<12} {'long prec':>10} {'long cov':>9} {'short prec':>11} {'short cov':>10}")
            for name, _m, p in variants:
                ...
                print(f"    {name:<12} {lp:>10.4f} {lm.mean():>9.4f} {sp:>11.4f} {sm.mean():>10.4f}")
