...
def test_config_requires_all_secrets():
    with pytest.raises(ProxyConfigError, match="OBSERVER_PROXY_TOKEN"):
        load_proxy_config({"LEDGER_INGEST_URL": "https://h", "LEDGER_INGEST_TOKEN": "t", "LEDGER_INGEST_SECRET": "s"})
    with pytest.raises(ProxyConfigError, match="LEDGER_INGEST_SECRET"):
        load_proxy_config({"OBSERVER_PROXY_TOKEN": "p", "LEDGER_INGEST_URL": "https://h", "LEDGER_INGEST_TOKEN": "t"})
    with pytest.raises(ProxyConfigError, match="https://"):
        load_proxy_config(
            {
                "OBSERVER_PROXY_TOKEN": "p",
                "LEDGER_INGEST_URL": "http://h",
...
def test_proxy_rejects_invalid_json_and_schema(monkeypatch):
    ...
    try:
        ...
        assert status == 422
        status, _ = _proxy_post(server, json.dumps({"producer": "mt5_observer", "events": "x"}).encode())
        assert status == 422
        status, _ = _proxy_post(server, json.dumps(_observer_envelope(producer="python_sender")).encode())
        assert status == 422
        status, _ = _proxy_post(server, json.dumps(_observer_envelope(mode="real")).encode())
        assert status == 422
...
def test_proxy_never_logs_secrets(caplog):
    ...
    with caplog.at_level(logging.INFO):
        ...
        proxy_mod.logger.info("forward producer=mt5_observer account_mode=demo events=1 batch=abc remote_status=200")
