...
def test_weekend_tag_on_saturday_not_checked(tmp_path):
    ...
    _make_csv(
        tmp_path,
        [
            {"entry_ts": ts, "session": "weekend", "direction": "long", "R": "0.1", "exit_reason": "breakeven"},
        ],
        asset="GBPUSD",
    )
...
def test_weekend_tag_on_monday_not_checked(tmp_path):
    ...
    _make_csv(
        tmp_path,
        [
            {"entry_ts": ts, "session": "weekend", "direction": "short", "R": "-0.5", "exit_reason": "stop"},
        ],
        asset="EURUSD",
    )
    assert audit_weekend_tags(str(tmp_path)) == []
...
def test_weekend_tag_before_21_not_checked(tmp_path):
    ...
    _make_csv(
        tmp_path,
        [
            {"entry_ts": ts, "session": "weekend", "direction": "short", "R": "-0.1", "exit_reason": "breakeven"},
        ],
        asset="XAGUSD",
    )
    assert audit_weekend_tags(str(tmp_path)) == []
...
def test_main_exit_code(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "scripts.audit_weekend_tags.audit_weekend_tags",
        lambda log_dir: [
            {
                "asset": "EURUSD",
                "csv": "x",
...
def test_main_exit_code_clean(monkeypatch):
    ...
    monkeypatch.setattr("scripts.audit_weekend_tags.audit_weekend_tags", lambda log_dir: [])
