    ...
    df = pd.DataFrame(
        {
            ...
            "atr": [0.0014] * 3,
            "regime": ["trend_up"] * 3,
            "ml_p_long": [0.9, 0.5, 0.6],
            "ml_p_short": [0.1, 0.5, 0.4],
        }
    )
...
def test_run_analysis_cost_stress_and_gate(synthetic_gbp_df):
    ...
    cfg = load_config()
    res = run_analysis(
        cfg, "GBPUSD", synthetic_gbp_df, variants={"current": {}, "null": None}, historical_trials=729, cost_stress=True
    )
    st = res["cost_stress"]
    ...
    fh = gate["fold_health"]
    assert {"total_pnl_positive", "ex_best_positive", "positive_share_ok", "passed"} <= set(fh)
    assert fh["passed"] == (fh["total_pnl_positive"] and fh["ex_best_positive"] and fh["positive_share_ok"])
...
def test_run_analysis_no_cost_stress_flag(synthetic_gbp_df):
    ...
    cfg = load_config()
    res = run_analysis(
        cfg, "GBPUSD", synthetic_gbp_df, variants={"current": {}}, historical_trials=729, cost_stress=False
    )
