...
def load_events(db_events=None, calendar_path: str | None = None, synthetic: bool = False, seed: int = 0) -> list[dict]:
    """Events as [{timestamp_utc, title}]. Priority: synthetic > CSV > live API."""
    if synthetic:
        rng = np.random.default_rng(seed)
        base = 1_700_000_000
        return [
            {"timestamp_utc": int(base + rng.integers(0, 400 * 86400)), "title": f"Synthetic event {i}"}
            for i in range(200)
        ]
    if calendar_path and os.path.exists(calendar_path):
        df = pd.read_csv(calendar_path)
        return [
            {"timestamp_utc": int(ts), "title": str(row.get("title", "event"))}
...
def run_event_tail(
    ...
    for fdf in frames:
        ...
        for t in engine.run(fdf.reset_index(drop=True)):
            ...
            rows.append({"entry_ts": int(t.entry_ts), "net_r": net_r, "exit_reason": t.exit_reason})
    ...
    tdf["minutes_to_event"] = tdf["entry_ts"].apply(lambda ts: _minutes_to_nearest(event_ts, int(ts)))
    tail = tail.copy()
    tail["minutes_to_event"] = tail["entry_ts"].apply(lambda ts: _minutes_to_nearest(event_ts, int(ts)))
    non_tail = non_tail.copy()
    non_tail["minutes_to_event"] = non_tail["entry_ts"].apply(lambda ts: _minutes_to_nearest(event_ts, int(ts)))
    ...
    def bucket_table(d: pd.DataFrame) -> list[dict]:
        ...
        for label, pred in BUCKETS:
            ...
            out.append({"bucket": label, "n": int(len(sel)), "share_pct": round(100.0 * len(sel) / len(d), 1)})
        return out
    ...
    if tail_share_in_30 >= 30.0:
        verdict = "HARD BLOCK justified: >30% of tail losses cluster inside ±30 min around Tier-1 events"
    ...
    return {
        ...
        "n_events": len(events),
        "tail_pct": tail_pct,
        "tail_threshold_r": round(thr, 4),
        "n_tail": int(len(tail)),
        "tail_share_in_30m_pct": round(tail_share_in_30, 1),
...
def print_report(d: dict) -> None:
    ...
    print(
        f"Trades: {d['n_trades']} | events: {d['n_events']} | tail = worst "
        f"{d['tail_pct']}% (netR <= {d['tail_threshold_r']}, n={d['n_tail']})"
    )
    if d["n_trades"] < 10:
        print("  insufficient trades.")
        return
    print(f"Tail share inside ±30m: {d['tail_share_in_30m_pct']}% vs all trades {d['all_share_in_30m_pct']}%")
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--synthetic-calendar", action="store_true", help="Use a synthetic event calendar (demo/tests only)"
    )
    ...
    try:
        ...
        synthetic = True
        print(
            f"[event] WARNING: cannot load real data ({exc.__class__.__name__}); SYNTHETIC demo — results are NOT real."
        )
    ...
    events = load_events(calendar_path=args.calendar, synthetic=args.synthetic_calendar or synthetic)
    if not events:
        print(
            "[event] WARNING: no calendar available (API offline / no --calendar). "
            "Use --calendar events.csv or --synthetic-calendar for a demo run."
        )
        return

    d = run_event_tail(cfg, args.asset, df, events, tail_pct=args.tail_pct, max_folds=args.max_folds)
