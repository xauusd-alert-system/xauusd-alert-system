...
def main() -> None:
    ...
    ap.add_argument("--bars", type=int, default=30000, help="recent bars to score per asset (0 = all)")
    ...
    print(f"min_edge={min_edge} min_ml_prob={min_ml_prob} ml_floor={ml_floor} rule_weight={w_rule} ml_weight={w_ml}")
    print("=" * 104)
    ...
    for asset in assets:
        ...
        if ab:
            ...
            print(
                f"# Platt sigmoid: a={a:.4f}  b={b:.4f}  crossover(raw p_long)={crossover:.4f}  "
        ...
        print(
            f"  {'raw':16s} {s_raw['mean']:7.4f} {s_raw['std']:7.4f} {s_raw['p5']:7.4f} "
            f"{s_raw['p50']:7.4f} {s_raw['p95']:7.4f} {100 * s_raw['share_gt_0.5']:6.1f}%"
        )
        print(
            f"  {'calibrated':16s} {s_cal['mean']:7.4f} {s_cal['std']:7.4f} {s_cal['p5']:7.4f} "
        ...
        print(
            f"  pass min_edge ({min_edge}):        {n2:>8d} ({100 * n2 / n0:5.1f}%)  "
        ...
        print(
            f"  rule_vote from regime: +1(up) {int((rule_vote == 1).sum())} / -1(down) "
        ...
        if ep.sum():
            ...
            print(
                f"  hard opposition on edge-pass: {int(opp.sum())} bars "
        ...
        rows.append(
            {
                ...
                "tf": tf,
                "bars": len(df),
                "sigmoid_a": ab[0] if ab else None,
                "sigmoid_b": ab[1] if ab else None,
                "raw_std": s_raw["std"],
                "cal_std": s_cal["std"],
                "raw_share_long": s_raw["share_gt_0.5"],
                "cal_share_long": s_cal["share_gt_0.5"],
                "edge_pass_n": n2,
                "edge_pass_long_n": n2_long,
