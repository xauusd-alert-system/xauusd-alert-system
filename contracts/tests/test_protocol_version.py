...
def _observer_envelope_dict():
    ...
    return build_envelope([event], producer="mt5_observer", account_mode="demo", account_login=7).model_dump(
        mode="json"
    )
...
def test_proxy_rejects_unknown_protocol_version():
    ...
    raw = build_envelope([event], producer="mt5_observer", account_mode="demo", account_login=7).model_dump(mode="json")
