...
def init_paper_schema(db_path: str) -> None:
    ...
    try:
        ...
        conn.execute(
            f"CREATE INDEX IF NOT EXISTS idx_{EVENTS_TABLE}_run_event "
            f"ON {EVENTS_TABLE}(run_id, event_id)"
        )
        conn.execute(
            f"CREATE INDEX IF NOT EXISTS idx_{EVENTS_TABLE}_trade "
...
def register_paper_run(db_path: str, manifest: dict) -> None:
    ...
    required = {
        "run_id", "asset_key", "variant", "manifest_sha256", "model_sha256",
        "start_timestamp_utc", "min_closed_trades", "created_at_utc",
    }
    ...
    try:
        ...
        if existing:
            if existing[0] != manifest["manifest_sha256"]:
                raise RuntimeError(
                    f"run_id {manifest['run_id']!r} is already registered with a different manifest"
                )
            return
        conn.execute(
            f"INSERT INTO {RUNS_TABLE} VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                manifest["run_id"], manifest["asset_key"], manifest["variant"],
                manifest["manifest_sha256"], manifest["model_sha256"],
                int(manifest["start_timestamp_utc"]), int(manifest["min_closed_trades"]),
                manifest["created_at_utc"], manifest_json,
            ),
        )
        conn.commit()
    finally:
...
def append_paper_event(
    ...
    *,
    ...
    try:
        run = conn.execute(
            f"SELECT 1 FROM {RUNS_TABLE} WHERE run_id=?", (run_id,)
        ).fetchone()
        if not run:
...
def read_paper_events(
    db_path: str, run_id: str, *, event_type: str | None = None,
...
def get_paper_run(db_path: str, run_id: str) -> dict:
    ...
    try:
        row = conn.execute(
            f"SELECT manifest_json FROM {RUNS_TABLE} WHERE run_id=?", (run_id,)
        ).fetchone()
    finally:
        conn.close()
    if not row:
