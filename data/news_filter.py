...
_FETCH_FAILED_UNTIL: float = 0.0  # reset on module reload
_LAST_FETCH_OK: bool | None = None
_LAST_ERROR: str | None = None
_LAST_SOURCE: str | None = None
CACHE_TTL_SECONDS = 3600  # 1 час кэша

_FF_NFS_URL = "https://nfs.faireconomy.media/ff_calendar_thisweek.json"
...
_LAST_FF_CALL_TS: float = 0.0  # throttle: min 60s between FF API calls


def _fetch_ff_json() -> tuple[List[Dict], str]:
    """Forex Factory JSON API (nfs.faireconomy.media). Returns (events, source)."""
    global _LAST_FF_CALL_TS
    now = time.time()
    if now - _LAST_FF_CALL_TS < 60:
        # Throttled: return cached data instead of hitting 429
        if _NEWS_CACHE:
            return _NEWS_CACHE, "forexfactory_json_api_cached"
        raise RuntimeError("FF API throttled, no cache yet")
    _LAST_FF_CALL_TS = now
    headers = {"User-Agent": _FF_UA}
    proxy = get_env("NEWS_FEED_PROXY", default="")
    proxies = {"http": proxy, "https": proxy} if proxy else None
    resp = requests.get(_FF_NFS_URL, headers=headers, timeout=10, proxies=proxies, verify=False)
    if resp.status_code == 429:
        # Rate limited: use cache if available, don't block
        if _NEWS_CACHE:
            return _NEWS_CACHE, "forexfactory_json_api_cached"
        raise RuntimeError(f"FF API 429 and no cache available")
...
def fetch_economic_calendar() -> List[Dict]:
    ...
    attempts: List[tuple] = []
    if method == "browser":
        # 1) JSON API (faireconomy.media CDN — fast, reliable),
        # 2) Shared background browser service (Playwright/Chromium).
        attempts = [(_fetch_ff_json,), (_fetch_ff_service,)]
    else:
    ...
    # If 429 rate limit: fail-open (don't block trading for news)
    is_rate_limit = "429" in str(last_error) or "Too Many Requests" in str(last_error)
    if is_rate_limit:
        _FETCH_FAILED_UNTIL = now + 300  # retry in 5 minutes, not 30
        logger.info("news feed rate-limited (429): using cache, retry in 5min")
    else:
        _FETCH_FAILED_UNTIL = now + 1800  # other errors: block 30 min
