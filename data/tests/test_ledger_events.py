...
def test_read_filters(ledger_db):
    ...
    upsert_ledger_event(ledger_db, _fact(
        event_id=execution_event_id("mt5_observer", "demo:7", "deal", "2"),
        deal_ticket=2, event_type="order_history_added",
        source="mt5_python_sender", precision="request"), signature_valid=True)
...
def test_execution_quality_summary_splits_precision(ledger_db):
    ...
    for deal, precision, spread, fill, req in (
        ...
        upsert_ledger_event(ledger_db, _fact(
            ...
            deal_ticket=deal, precision=precision, spread_points=spread,
            fill_price=fill, requested_price=req,
...
def test_lifecycle_trace(ledger_db):
    ...
    upsert_ledger_event(ledger_db, _fact(
        ...
        source="mt5_python_sender", precision="request",
        volume_requested=0.1, received_at_utc_ms=1_700_000_000_400), signature_valid=True)
    upsert_ledger_event(ledger_db, _fact(
        event_id=execution_event_id("mt5_observer", "demo:7", "deal", "9"),
        intent_id=intent_id, deal_ticket=9, precision="passive",
        received_at_utc_ms=1_700_000_000_500), signature_valid=True)
...
def test_payload_roundtrip(ledger_db):
    upsert_ledger_event(ledger_db, _fact(payload={"reconciled": True, "entry": 0}),
                        signature_valid=True)
