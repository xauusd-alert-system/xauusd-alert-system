...
def purge_train_indices(train_idx: np.ndarray, test_start: int, test_end: int, horizon: int) -> np.ndarray:
...
def embargo_train_indices(train_idx: np.ndarray, test_bound: int, embargo: int) -> np.ndarray:
...
def purged_kfold_indices(n: int, n_splits: int, horizon: int, embargo: int = 0) -> list[tuple[np.ndarray, np.ndarray]]:
