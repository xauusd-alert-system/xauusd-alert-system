...
class MT5TradeGroupExecutor:
    ...
    def _verify_compensation(self, group: dict[str, Any]) -> str:
        return comp.verify_compensation(self, group)
    ...
    def _stop_group(self, group: dict[str, Any], stop_price: float) -> str:
        return tp_transitions.stop_group(self, group, stop_price)

    # ------------------------------------------------------------------
    # BE flow (ТЗ §17–§20): actual fill, all apply_to legs, broker query.
    # P2-1.1: bodies live in execution.mt5_be_flow; thin delegates remain
    # for backward compatibility (tests may patch/mock them).
    # ------------------------------------------------------------------

    def _request_be(self, group: dict[str, Any]) -> str:
        return be_flow.request_be(self, group)

    def _verify_be(self, group: dict[str, Any]) -> str:
        return be_flow.verify_be(self, group)
