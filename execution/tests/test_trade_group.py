...
def test_geometry_direction_consistent():
    ...
    with pytest.raises(ValidationError):
        make_spec(side="long", geometry={**make_spec().geometry.model_dump(), "tp1": 4150.0})
    with pytest.raises(ValidationError):
        make_spec(side="long", geometry={**make_spec().geometry.model_dump(), "sl": 4200.0})
    with pytest.raises(ValidationError):
        make_spec(side="long", geometry={**make_spec().geometry.model_dump(), "tp2": 4160.0})  # below tp1
    # short mirror
    short = make_spec(
        side="short",
        entry={**make_spec().entry.model_dump(), "reference": 4159.30, "low": 4159.10, "high": 4159.50},
        geometry={
            "version": "v",
            "unit": "price",
            "step_price": 4.30,
...
def test_volume_allocation_floor_rule_matches_spec_example():
    ...
    volumes = allocate_leg_volumes(0.05, (1 / 3, 1 / 3, 1 / 3), volume_step=0.01, volume_min=0.01)
...
def test_volume_allocation_rejects_unfillable_legs():
    ...
    with pytest.raises(ValueError, match="INSUFFICIENT_VOLUME_FOR_THREE_LEGS"):
        allocate_leg_volumes(
            0.03,
            (0.1, 0.1, 0.8),  # leg2 = floor(0.003) = 0
            volume_step=0.01,
            volume_min=0.01,
        )
    # explicit netting fallback is allowed and visible
    volumes = allocate_leg_volumes(0.03, (0.1, 0.1, 0.8), volume_step=0.01, volume_min=0.01, allow_short_legs=True)
...
def test_group_risk_single_cap_not_per_leg():
    ok, reason = check_group_risk(estimated_loss_at_sl=24.0, max_cash=25.0, max_pct=0.01, balance=10000.0)
    assert ok is True and reason is None
    # 3x intended risk must fail (simulating per-leg miscalc)
    ok, reason = check_group_risk(estimated_loss_at_sl=72.0, max_cash=25.0, max_pct=0.01, balance=10000.0)
    assert ok is False and reason == "RISK_LIMIT_EXCEEDED"
    # pct cap also enforced
    ok, reason = check_group_risk(estimated_loss_at_sl=200.0, max_cash=10000.0, max_pct=0.01, balance=10000.0)
    assert ok is False and reason == "RISK_LIMIT_EXCEEDED"
...
def test_gross_r_example_form():
    spec = make_spec()
    r = calculate_gross_r(spec.side, spec.entry.reference, spec.geometry.sl, spec.targets)
...
def test_allocation_sums_to_total_for_both_directions(total):
    ...
    spec_long = _spec_100(
        risk={"currency": "USD", "max_cash": 50.0, "max_pct": 0.5, "estimated_loss_at_sl": 48.0, "total_volume": total}
    )
    spec_short = _short_100(
        risk={"currency": "USD", "max_cash": 50.0, "max_pct": 0.5, "estimated_loss_at_sl": 48.0, "total_volume": total}
    )
    for spec in (spec_long, spec_short):
        allocated = [round(spec.risk.total_volume * t.allocation, 8) for t in spec.targets]
...
def test_allocate_small_volume_dust():
    ...
    volumes = allocate_leg_volumes(0.03, (1 / 3, 1 / 3, 1 / 3), volume_step=0.01, volume_min=0.01)
    assert volumes == [0.01, 0.01, 0.01]
...
def test_allocate_negative_leg3_rejected():
    ...
    with pytest.raises(ValueError, match="INSUFFICIENT_VOLUME_FOR_THREE_LEGS"):
        allocate_leg_volumes(0.05, (0.8, 0.8, 0.4), volume_step=0.01, volume_min=0.01, allow_short_legs=True)


def test_allocate_negative_leg3_rejected_strict_mode():
    """P1-11: same rejection holds without the netting fallback flag."""
    with pytest.raises(ValueError, match="INSUFFICIENT_VOLUME_FOR_THREE_LEGS"):
        allocate_leg_volumes(0.05, (0.8, 0.8, 0.4), volume_step=0.01, volume_min=0.01)
...
def test_estimated_loss_same_for_long_and_short():
    ...
    long_out = calculate_geometry(
        profile=profile, side="long", reference_price=4159.30, atr=8.0, broker=broker, cost=cost
    )
    short_out = calculate_geometry(
        profile=profile, side="short", reference_price=4159.30, atr=8.0, broker=broker, cost=cost
    )
...
def test_atr_change_does_not_touch_approved_spec():
    ...
    cfg = {
        "trade_profiles": {
            "p": {
                ...
                "risk": {"max_cash": 200.0, "max_pct": 0.5},
                "volume": {"total": 0.06},
            }
        }
    }
    ...
    signal = {"bias": "long", "atr": 4.0, "entry_zone": [4159.10, 4159.50], "expires_at_utc_ms": 1_900_000_000_000}
