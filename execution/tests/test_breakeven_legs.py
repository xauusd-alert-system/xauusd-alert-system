...
def _fake_world(monkeypatch, trader, positions, deals_by_ticket=None, symbol_ticks=None):
    ...
    def fake_order_send(request):
        ...
        return types.SimpleNamespace(
            retcode=trader_mod.mt5.TRADE_RETCODE_DONE,
            comment="done",
            order=0,
        )
    ...
    monkeypatch.setattr(
        trader_mod.mt5, "history_deals_get", lambda position=None: (deals_by_ticket or {}).get(position, [])
    )
...
def test_gold_long_be_reaches_true_entry_and_returns_true(monkeypatch, tmp_path):
    ...
    t = _trader(tmp_path, {11: dict(LEG2, symbol="XAUUSD", type="long", entry_price=4393.03)})
...
def test_close_detector_be_block_marks_done_on_real_acceptance(monkeypatch, tmp_path):
    ...
    t = _trader(
        tmp_path,
        {
            1: dict(LEG2, symbol="XAUUSD", type="long", leg=1, entry_price=4393.03),
            3: dict(LEG3, symbol="XAUUSD", type="long", entry_price=4393.03),
        },
    )
    sends = _fake_world(
        monkeypatch,
